...
public class ListaAlocacaoFlexivel {
    public static void main(String[] args) throws Exception {
        ...
        int i = 0;
        ListaDupla listaFlexJogador = new ListaDupla();
        ...
        while (!entrada.equals("FIM")) {
            for (int j = 0; j < jogador.length; j++) {
                ...
                if (jogador[j].getId().toString().equals(entrada)) {
                    listaFlexJogador.inserirFim(jogador[j]);
        ...
        for(int j = 0; j < limite; j++){
            ...
            switch(instrucao){
                case "II":
                    ...
                    listaFlexJogador.inserirInicio(retornaBuildado(idAdd, jogador));
                    break;
                case "IF":
                    idAdd = Integer.parseInt(entrada.substring(3));
                    listaFlexJogador.inserirFim(retornaBuildado(idAdd, jogador));
                    break;
                case "I*":
                    pos = Integer.parseInt(entrada.split(" ")[1]);
                    idAdd = Integer.parseInt(entrada.split(" ")[2]);
                    listaFlexJogador.inserir(retornaBuildado(idAdd, jogador), pos);
                    break;
                case "RI":
                    System.out.println("(R) " + listaFlexJogador.removerInicio().getNome());
                    break;
                case "RF":
                    System.out.println("(R) " + listaFlexJogador.removerFim().getNome());
                    break;
                case "R*":
                    pos = Integer.parseInt(entrada.split(" ")[1]);
                    System.out.println("(R) " + listaFlexJogador.remover(pos).getNome());
                    break;
                default:
