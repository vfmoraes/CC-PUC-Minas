...
int minSelection(Jogador arr[], int comeco, int fim) {
    int minIndex = comeco;

    for(int i = comeco+1; i <= fim; i++) {
        comparacoes++;
        if (strcmp(arr[i].nome, arr[minIndex].nome) < 0) {
            minIndex = i;
        }
    }
    return minIndex;
...
void SelectionSortRecursivo(Jogador arr[], int comeco, int fim) {
    if(comeco < fim) {
        int minIndex = minSelection(arr, comeco, fim);
        troca(&arr[comeco], &arr[minIndex]);
        SelectionSortRecursivo(arr, comeco+1, fim);
    }
}
...
int main()
{
    ...
    SelectionSortRecursivo(jogadores, 0, numJogadores - 1);
