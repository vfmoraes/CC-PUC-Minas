...
void countingSort(Jogador arr[], int n, int exp, int *comparacoes, int *movimentacoes)
{
  Jogador output[n];
  int i, count[10] = {0};
...
void radixSort(Jogador arr[], int n)
{
  ...
  for (int exp = 1; max_id / exp > 0; exp *= 10)
    countingSort(arr, n, exp, &comparacoes, &movimentacoes);
}
...
int main()
{
  ...
  radixSort(jogadores, numJogadores);
