...
void insertionSort(Jogador arr[], int comeco, int fim)
{
  ...
  for (i = comeco + 1; i <= fim; i++)
  {
    key = arr[i];
    j = i - 1;
    while (j >= comeco && (arr[j].anoNascimento > key.anoNascimento || (arr[j].anoNascimento == key.anoNascimento && strcmp(arr[j].nome, key.nome) > 0)))
    {
      arr[j + 1] = arr[j];
      j = j - 1;
      movimentacoes+=2;
...
int main()
{
  ...
  insertionSort(jogadores,0, numJogadores - 1);
