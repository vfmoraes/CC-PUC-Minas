...
{
    Jogador pivo = arr[high];
    int i = (low - 1);

    for (int j = low; j <= high - 1; j++)
    {
        comparacoes++;
        if (strcmp(arr[j].estadoNascimento, pivo.estadoNascimento) < 0 ||
            (strcmp(arr[j].estadoNascimento, pivo.estadoNascimento) == 0 && strcmp(arr[j].nome, pivo.nome) < 0))
        {
            i++;
            troca(&arr[i], &arr[j]);
        }
