...
public class JogadorSelecao implements Cloneable, Comparable<JogadorSelecao> {
  ...
  public JogadorSelecao() {
    // Inicializa o objeto sem definir valores para os atributos
  }

  // Construtor com todos os atributos como argumentos
  public JogadorSelecao(int id, String nome, int altura, int peso, String universidade, int anoNascimento,
  ...
  @Override
  public JogadorSelecao clone() throws CloneNotSupportedException {
    return (JogadorSelecao) super.clone();
  }

  // Método que permite a comparação entre dois Jogadores pelo nome, necessário para a ordenação
  @Override
  public int compareTo(JogadorSelecao outro) {
  ...
  public static void ordenarPorSelecao(JogadorSelecao[] jogadores, String matricula) {
    // Inicialização de contadores e tempo de execução
    int numComparacoes = 0;
    int numMovimentacoes = 0;
    long tempoInicial = System.currentTimeMillis();

    // Loop principal do algoritmo de ordenação por seleção
    for (int i = 0; i < jogadores.length - 1; i++) {
      ...
      numMovimentacoes += 3;
      JogadorSelecao temp = jogadores[i];
  ...
  // Método para salvar as informações de log (número de comparações e movimentações, tempo de execução) em um arquivo
  public static void salvarLog(String matricula, int numComparacoes, int numMovimentacoes, long tempoExecucao) {
    try {
      PrintWriter pw = new PrintWriter(new FileWriter("matricula_selecao.txt"));
  ...
  public static void main(String[] args) {
    ...
    ArrayList<JogadorSelecao> jogadores = new ArrayList<>();
    String id;

    // Loop que continua lendo a entrada do usuário até que a palavra "FIM" seja
    // inserida
    while (!(id = input.nextLine()).equals("FIM")) {
      JogadorSelecao jogador = new JogadorSelecao(); // Cria um novo objeto Jogador
      jogador.ler(id); // Lê os atributos do jogador a partir do arquivo CSV
      jogadores.add(jogador); //Adiciona o jogador ao ArrayList
    }

    JogadorSelecao[] arrayJogadores = jogadores.toArray(new JogadorSelecao[0]);
    ordenarPorSelecao(arrayJogadores, "802371");

    for (JogadorSelecao jogador : arrayJogadores) {
