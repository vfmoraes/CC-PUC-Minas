...
public class JogadorPesquisaSequencial implements Cloneable {
  ...
  public JogadorPesquisaSequencial() {
    // Inicializa o objeto sem definir valores para os atributos
  }

  // Construtor com todos os atributos como argumentos
  public JogadorPesquisaSequencial(int id, String nome, int altura, int peso, String universidade, int anoNascimento,
  ...
  @Override
  public JogadorPesquisaSequencial clone() throws CloneNotSupportedException {
    return (JogadorPesquisaSequencial) super.clone();
  }
  ...
  public static void main(String[] args) {
    List<JogadorPesquisaSequencial> jogadores = new ArrayList<>();
    ...
    while (!(id = input.nextLine()).equals("FIM")) {
      JogadorPesquisaSequencial jogador = new JogadorPesquisaSequencial();
      jogador.ler(id);
    ...
    while (!(id = input.nextLine()).equals("FIM")) {
      ...
      int resultado = pesquisaSequencial(jogadores, id, contComparacoes);
    ...
    try {
      FileWriter fileWriter = new FileWriter("matricula_sequencial.txt", false);
      PrintWriter printWriter = new PrintWriter(fileWriter);
      printWriter.printf("%s\t%d\t%d\n", matricula, tempoExecucao, JogadorPesquisaSequencial.contComparacoes);
      printWriter.close();
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  // Método para realizar a pesquisa sequencial
  public static int pesquisaSequencial(List<JogadorPesquisaSequencial> jogadores, String nome, int contComparacoes) {
    for (int i = 0; i < jogadores.size(); i++) {
      JogadorPesquisaSequencial.contComparacoes++;
